...
public final class Autos {
  ...
  public static Command ForwardTaxiBalance(SwerveDriveSubsystem subsystem, ArmSubsystem armSubsystem,
      ClawSubsystem clawSubsystem) {
    return ( PlaceMiddle(armSubsystem, clawSubsystem)
    .andThen(new DriveXDistanceForward(subsystem, -2, 0, 1, 1))) //- (1.5 + .355 + 1.22 + .355 + .75)
    .andThen(new WaitCommand(0.3))
    .andThen(new DriveXDistanceForward(subsystem, -1.65, 0, 1, 1))
    .andThen(new WaitCommand(0.2))
    .andThen(new DriveXDistanceForward(subsystem, 1.45, 0) //+ .355 + 75
    .andThen(new SelfBalance(subsystem)));
  ...
  public static Command PlaceMiddle(ArmSubsystem armSubsystem, ClawSubsystem clawSubsystem) {
    return Constants.ArmCommands.MoveArmToMiddle(armSubsystem)
        ...
        .andThen(
          (Constants.ArmCommands.moveArmIdle(armSubsystem)).alongWith(new ClawControl(clawSubsystem, false))
        );
  }
  ...
  public static Command GrabFromSubation(ArmSubsystem armSubsystem, ClawSubsystem clawSubsystem) {
    return new ClawControl(clawSubsystem, true)
        ...
        .andThen(
          (new ClawControl(clawSubsystem, false)).alongWith(Constants.ArmCommands.moveArmIdle(armSubsystem))
        );
  }
  ...
  public static CommandBase LowerPlaceAndBalanceCone(SwerveDriveSubsystem swerveDriveSubsystem,
    ...
    return PlaceMiddle(armSubsystem, clawSubsystem)
        ...
        .andThen(
          (new ClawControl(clawSubsystem, false)).alongWith(new SelfBalance(swerveDriveSubsystem))
        );
  }
